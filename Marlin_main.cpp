...
void setup()
{
 ...
 enquecommand_P(PSTR("G1 X0 Y105"));
 #ifdef RECOVER_PUMP
 fan_on = false;
 fan_reconver = false;
 #endif
  ...
  if(code_seen('G'))
  {
    ...
    case 4: // G4 dwell
      ...
      if(code_seen('P')){ 
        codenum = code_value();
        #ifdef DEBUG_STONE
        SERIAL_ECHOPAIR(" P",codenum);
        #endif
        } // milliseconds to wait
      if(code_seen('S')){ 
        codenum = code_value() * 1000; 
        #ifdef DEBUG_STONE
        SERIAL_ECHOPAIR(" S",codenum);
        #endif
        }// seconds to wait
    ...
    SERIAL_PROTOCOL("\nG28");
      ...
      #ifdef DEBUG_STONE
      SERIAL_PROTOCOLPGM("\nStart time");
      SERIAL_PROTOCOL(previous_millis_cmd);
      #endif
      enable_endstops(true);
...
void get_coordinates()
{
  ...
  for(int8_t i=0; i < NUM_AXIS; i++) {
    if(code_seen(axis_codes[i]))
    {
      ...
      #ifdef DEBUG_STONE
        if(i==0)
            SERIAL_ECHOPAIR(" X",destination[i]);
        else if(i == 1)
            SERIAL_ECHOPAIR(" Y",destination[i]);
        else if(i == 2)
            SERIAL_ECHOPAIR(" Z",destination[i]);
        else if(i == 3)
            SERIAL_ECHOPAIR(" E",destination[i]);
      #endif
    }
  ...
  if(code_seen('F')) {
    ...
    #ifdef DEBUG_STONE
       SERIAL_ECHOPAIR(" F",next_feedrate);
    #endif
  }
}
