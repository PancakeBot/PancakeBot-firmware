...
unsigned long message_milliss = 0;
uint8_t speeddis =0;
...
uint32_t Buttoncounter1;
uint32_t Buttoncounter2;
uint32_t Buttoncounter3;
uint32_t Buttoncounter4;
  ...
  static  int back = 0;
    ...
    if(LCD_CLICKEDD){
     if(B_poweronflag){
       if (!card.sdprinting){
           B_poweronflag =  false;
           WRITE(LED_POWEROFF,HIGH); // Standby 
           WRITE(LED_POWERON,LOW); // Standby 
           ...
           B_poweronflag =  true;
           WRITE(LED_POWERON,HIGH); // Standby 
           WRITE(LED_POWEROFF,LOW); // Standby 
           lcd_implementation_init( // to maybe revive the LCD if static electricity killed it.
              #if defined(LCD_PROGRESS_BAR) && defined(SDSUPPORT)
                currentMenu == lcd_status_screen
              #endif
...
#if 0//defULTIPANEL_FEEDMULTIPLY
    ...
    if(speeddis == 1){
        if(millis() - message_milliss > 5000){
            speeddis  = 0;
        }
    }
#ifdef DEBUG_STONE
    if(back!=feedmultiply ){
        back = feedmultiply;
        SERIAL_PROTOCOLPGM("\nspeed");
...
void lcd_sdcard_menu()
{
    ...
    if (lcdDrawUpdate == 0 && wasClicked == 0)
        return;
#endif
...
void lcd_init()
{
    pinMode(DOGLCD_BACK,OUTPUT);
    WRITE(DOGLCD_BACK,HIGH);
    
    //pinMode(LED_POWERON,OUTPUT);
    //pinMode(LED_POWEROFF,OUTPUT);
    pinMode(LED_SDERROR,OUTPUT);
    pinMode(LED_WORKING,OUTPUT);

    //WRITE(LED_POWERON,LOW);
    //WRITE(LED_POWEROFF,HIGH);
    WRITE(LED_SDERROR,LOW);
    WRITE(LED_WORKING,LOW);
...
void lcd_update()
{
    ...
    static unsigned long timeoutTopoweroff = 0;
    static unsigned int timeoff = 0;
    ...
    if(currentMenu != lcd_status_screen){
        ...
        if(card.sdprinting||card.sdpouse||blocks_queued()){
            poweroff = 0;
            timeoff = 0;
        }
        else{
            if(poweroff == 0){
                poweroff = 1;
                timeoutTopoweroff = millis() + 1000;
            }
            else if(poweroff == 1){
                if(millis() > timeoutTopoweroff){
                    timeoutTopoweroff = millis() + 1000;
                    timeoff ++;
                    if(timeoff >= 300){
                            poweroff = 0;
...
void lcd_buttons_update()
{
#ifdef REPRAP_128X32GRAPH_CONTROLLER
    uint8_t newbutton=0;
    static uint8_t BTN_ENB1 = 0;
    static uint8_t BTN_ENB2 = 0;
    static uint8_t BTN_ENB3 = 0;
    static uint8_t BTN_ENB4 = 0;
   /*
  ...
  */
        ...
        if(READ(BTN_ENC)==0){
            if(BTN_ENB3 == 0){
                    BTN_ENB3 = 1;
                    Buttoncounter3 = millis();
            }
        }
        else{
            if(BTN_ENB3 ==  1){
                   if(millis()>(Buttoncounter3+20)){
                            buttons |= EN_C;
                   }
                   BTN_ENB3 = 0;
            }
        ...
        if(READ(BTN_END)==0){
            if(BTN_ENB4 == 0){
                    BTN_ENB4 = 1;
                    Buttoncounter4 = millis();
            }
        }
        else{
            if(BTN_ENB4 ==  1){
                   if(millis()>(Buttoncounter4+20)){
                            buttons |= EN_D;
                   }
                   BTN_ENB4 = 0;
            }
        ...
        if(READ(BTN_EN2)==0){
            if(BTN_ENB2 == 0){
                    BTN_ENB2 = 1;
                    Buttoncounter1 = millis();
            }
        }
        else{
            if(BTN_ENB2 ==  1){
                   if(millis()>(Buttoncounter1+20)){
                            encoderDiff = 5;
                   }
                   BTN_ENB2 = 0;
            }
        ...
        if(READ(BTN_EN1)==0){
            if(BTN_ENB1 == 0){
                    BTN_ENB1 = 1;
                    Buttoncounter2 = millis();
            }
        }
        else{
            if(BTN_ENB1 ==  1){
                   if(millis()>(Buttoncounter2+20)){
                           encoderDiff = -5;
                   }
                   BTN_ENB1 = 0;
            }
